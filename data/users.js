...
module.exports = {
    async createUser(username, password, email, fName, lName, userType, phonenumber, dob) {
        try {
            ...
            if (dob.charAt(1)!=="9" && dob.charAt(1)!=="0"){ throw [400, `Enter valid Date`]; }
            if (dob.charAt(0)!=="1" && dob.charAt(0)!=="2"){ throw [400, `Enter valid Date`]; }
            if ((dob.charAt(0) ==="2"|| dob.charAt(1) ==="0") && (dob.charAt(2)!=="0" && dob.charAt(2)!=="1")){ throw [400, `Enter valid Date`]; }
            else {
...
if(!dateofbirth) throw `Error: You must supply date of birth`;
if(dateofbirth[6]<9) throw `Error: Enter valid date`;
}