...
router.get("/meeting/:room/:pass", async (req, res) => {
  if (!req.session.user) {
    ...
    try {
      ...
      if (chckMeet.authenticated) {
        const addUserToMeet = await meetData.updateMeet(
          meetId,
...
router.post("/signup", async (req, res) => {
  try {
    ...
    const adduser = await usersData.createUser(
      usernameSign,
      passwordSign,
      email,
      fName,
...
router.post("/editprofile", upload.single("profilePic"), async (req, res) => {
  ...
  try {
    ...
    const edituser = await usersData.editUser(
...
const checkCreateUser = function checkCreateUser(user, pass) {
  ...
  if (typeof user !== "string")
    throw [400, `Please pass only characters in an Username`];
  if (!user.replace(/\s/g, "").length)
    throw [400, `Please don't pass only white spaces`];
  if (!/^[a-zA-Z]+$/.test(user))
    throw [400, `Please input only charaters in an Username`];
  if (user.length < 4)
    throw [400, `Please enter a valid username(atleast 4 characters long)`];
  if (typeof pass !== "string")
    throw [400, `Please pass only characters in an password`];
  if (!pass.replace(/\s/g, "").length)
    throw [400, `Please don't pass white spaces in password`];
  if (/\s/.test(pass))
    throw [400, `Please input only charaters in an Password`];
  if (pass.length < 6)
    throw [400, `Please enter a valid password(atleast 6 characters long)`];
};
